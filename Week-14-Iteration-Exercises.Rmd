...
```{r}
#remember the pluck function...this note has nothing to do with following code
exams %>% map(mean, na.rm=TRUE)
#Get back vector instead of list:
exams %>% map(mean, na.rm=TRUE)
```
...
```{r}
haha<-function(x,y) (mean(x) + y)

exams %>% 
  map2_dbl(extra_credit, ~haha(.x,.y))
```
